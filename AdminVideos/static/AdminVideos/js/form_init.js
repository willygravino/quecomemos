 ...
 * evitando la API vieja que persistía por nombre.
 */
if (!document.__platoIngredientesToggleBound) {
  console.log("✅ platoIngredientesToggleBound cargado (ID-based)");

  // ---- Guardar al tildar/destildar (checked = "hay que comprar" => "1") ----
  document.addEventListener("change", async (ev) => {
    ...
    const form = modal.querySelector("#frm-plato-ingredientes");
    if (!form) return;
    ...
    const commentInput = modal.querySelector(`input[name="comentario_${ingId}"]`);
  ...
  // ---- Guardar comentario mientras escribe (sin tocar el checkbox) ----
  document.addEventListener("input", async (ev) => {
    const inp = ev.target.closest("input.form-control[name^='comentario_']");
    if (!inp) return;
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data.success === false) {
        console.error("POST comment error:", res.status, data);
      }
    } catch (e) {
